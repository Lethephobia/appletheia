...
pub struct Erc2612PermitSignature([u8; 65]);

impl Erc2612PermitSignature {
...
impl Serialize for Erc2612PermitSignature {
...
impl<'de> Deserialize<'de> for Erc2612PermitSignature {
        ...
        D: Deserializer<'de>,
    {
        ...
        Self::decode_hex(&encoded)
            .ok_or_else(|| serde::de::Error::custom("invalid ERC-2612 permit signature"))
    }
}

#[cfg(test)]
mod tests {
    use super::Erc2612PermitSignature;

    #[test]
    fn stores_bytes_and_serializes_as_prefixed_hexadecimal() {
        let signature = Erc2612PermitSignature::from_bytes([0xab; 65]);
        let encoded = format!("\"0x{}\"", "ab".repeat(65));

        assert_eq!(signature.as_bytes(), &[0xab; 65]);
        assert_eq!(signature.to_hex(), format!("0x{}", "ab".repeat(65)));
        assert_eq!(
            serde_json::from_str::<Erc2612PermitSignature>(&encoded)
    ...
    fn rejects_invalid_signature_length() {
        assert!(serde_json::from_str::<Erc2612PermitSignature>("\"0x12\"").is_err());
    }
}
