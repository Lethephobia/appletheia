...
#[derive(Debug, Error)]
pub enum PgUserOrganizationJoinRequestListUserRowError {
    #[error("user organization join request list user row has an invalid user id")]
    UserId(#[source] Box<dyn std::error::Error + Send + Sync>),

    #[error("user organization join request list user row has an invalid username")]
    Username(#[source] Box<dyn std::error::Error + Send + Sync>),

    #[error("user organization join request list user row has an invalid display name")]
