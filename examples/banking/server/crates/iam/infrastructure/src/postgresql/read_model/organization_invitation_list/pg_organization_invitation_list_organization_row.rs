    ...
    fn try_from(row: PgOrganizationInvitationListOrganizationRow) -> Result<Self, Self::Error> {
        Ok(Self {
            organization_id: OrganizationId::try_from_uuid(row.organization_id).map_err(
                |error| {
                    PgOrganizationInvitationListOrganizationRowError::OrganizationId(Box::new(
                        error,
                    ))
                },
            )?,
            handle: OrganizationHandle::try_from(row.handle).map_err(|error| {
                PgOrganizationInvitationListOrganizationRowError::Handle(Box::new(error))
            })?,
            display_name: OrganizationDisplayName::try_from(row.display_name).map_err(|error| {
                PgOrganizationInvitationListOrganizationRowError::DisplayName(Box::new(error))
            })?,
            ...
            .into_picture()
            .map_err(|error| {
                PgOrganizationInvitationListOrganizationRowError::Picture(Box::new(error))
            })?,
            observation: ReadModelObservation::new(
                EventId::try_from(row.source_event_id).map_err(|error| {
                    PgOrganizationInvitationListOrganizationRowError::SourceEventId(Box::new(error))
                })?,
                EventId::try_from(row.updated_event_id).map_err(|error| {
                    PgOrganizationInvitationListOrganizationRowError::UpdatedEventId(Box::new(
                        error,
                    ))
                })?,
            ),
