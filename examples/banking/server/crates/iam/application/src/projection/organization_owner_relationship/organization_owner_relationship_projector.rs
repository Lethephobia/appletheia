    ...
    RS: RelationshipStore,
{
    ...
    async fn project(&self, uow: &mut Self::Uow, event: &EventEnvelope) -> Result<(), Self::Error> {
        ...
        let event = event.try_into_domain_event::<Organization>()?;

        self.relationship_store
            .apply_changes(
                uow,
                &[RelationshipChange::Upsert(Relationship {
                    aggregate: AggregateRef::from_id::<Organization>(event.aggregate_id()),
...
mod tests {
    ...
    use banking_iam_domain::{Organization, OrganizationName, User, UserId};
