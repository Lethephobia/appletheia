...
impl CurrencyDefinition {
    ...
    /// Changes the current decimal precision.
    pub fn change_decimals(
        &mut self,
        decimals: CurrencyDecimals,
    ) -> Result<(), CurrencyDefinitionError> {
        if self.state_required()?.decimals().eq(&decimals) {
            return Ok(());
        }

        self.append_event(CurrencyDefinitionEventPayload::DecimalsChanged { decimals })
    }
    ...
    for CurrencyDefinition
{
    fn apply(
        ...
        match payload {
            ...
            CurrencyDefinitionEventPayload::DecimalsChanged { decimals } => {
                self.state_required_mut()?.set_decimals(*decimals);
            }
...
mod tests {
    ...
    fn changing_to_same_values_and_same_status_is_a_no_op() {
        ...
        currency_definition
            ...
            .expect("no-op decimals change should succeed");
        currency_definition
            .activate()
    ...
    fn change_methods_append_events_and_update_state() {
        ...
        let changed_decimals = CurrencyDecimals::new(9);
        let mut currency_definition = CurrencyDefinition::default();
        currency_definition
        ...
        currency_definition
            .change_decimals(changed_decimals)
            .expect("decimals change should succeed");
        currency_definition
            .deactivate()
        ...
        assert_eq!(state.decimals(), &changed_decimals);
        assert!(!state.is_active());
        assert_eq!(currency_definition.uncommitted_events().len(), 5);
    }
