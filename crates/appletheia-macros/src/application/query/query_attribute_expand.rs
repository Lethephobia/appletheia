...
pub(crate) fn expand_query_attribute(
    ...
    let Item::Struct(ref mut item) = item else {
        return Err(syn::Error::new(
            item.span(),
            "`#[query(...)]` can only be applied to a struct",
        ));
    };
    ...
    let already_has_helper = item
        .attrs
        .iter()
        .any(|attr| attr.path().is_ident("query_derive"));
    if already_has_helper {
        return Err(syn::Error::new(
            item.span(),
            "`#[query_derive(...)]` must not be used together with `#[query(...)]`",
        ));
    }

    let existing_derive_keys = collect_derive_keys(&item.attrs)?;
    let macros_root = resolve_macros_root()?;
    let query_derive: Path = syn::parse_quote!(#macros_root::Query);

    let should_add_query_derive = derive_key(&query_derive)
        .map(|key| !existing_derive_keys.contains(&key))
        .unwrap_or(true);

    if should_add_query_derive {
        item.attrs.push(syn::parse_quote!(#[derive(#query_derive)]));
    }

    item.attrs
        .push(syn::parse_quote!(#[query_derive(#attr_tokens)]));
