...
pub(crate) fn expand_relations_attribute(
    ...
    let Item::Struct(ref mut item) = item else {
        return Err(syn::Error::new(
            item.span(),
            "`#[relations(...)]` can only be applied to a struct",
        ));
    };
    ...
    let already_has_helper = item
        .attrs
        .iter()
        .any(|attr| attr.path().is_ident("relations_derive"));
    if already_has_helper {
        return Err(syn::Error::new(
            item.span(),
            "`#[relations_derive(...)]` must not be used together with `#[relations(...)]`",
        ));
    }

    let existing_derive_keys = collect_derive_keys(&item.attrs)?;
    let macros_root = resolve_macros_root()?;
    let relations_derive: Path = syn::parse_quote!(#macros_root::Relations);

    let should_add_relations_derive = derive_key(&relations_derive)
        .map(|key| !existing_derive_keys.contains(&key))
        .unwrap_or(true);

    if should_add_relations_derive {
        item.attrs
            .push(syn::parse_quote!(#[derive(#relations_derive)]));
    }

    item.attrs
        .push(syn::parse_quote!(#[relations_derive(#attr_tokens)]));
