...
use appletheia_application::read_model::{
    ReadModelDependency, ReadModelFragment, ReadModelFragmentName, ReadModelInvalidationEnvelope,
    ReadModelObservation, ReadModelObservationSource,
};
...
use appletheia_infrastructure::postgresql::outbox::read_model_invalidation::PgReadModelInvalidationOutboxEnqueuer;
...
async fn enqueue_persists_dependency_only_invalidation(pool: PgPool) {
    ...
    let envelope = ReadModelInvalidationEnvelope::try_new(
        ...
        [ReadModelDependency::Partition(partition.clone())],
    )
    ...
    PgReadModelInvalidationOutboxEnqueuer::new()
        .enqueue_invalidations(&mut uow, &[envelope])
        .await
        .expect("read-model invalidations should enqueue");
    uow.commit().await.expect("uow should commit");

    let dependencies = sqlx::query_scalar::<_, serde_json::Value>(
        "SELECT invalidated_dependencies FROM read_model_invalidation_outbox",
    )
    .fetch_one(&pool)
    .await
    ...
    assert_eq!(
        dependencies,
        serde_json::to_value([ReadModelDependency::Partition(partition)]).unwrap()
    );
}

#[sqlx::test(migrations = "migrations/postgresql")]
#[ignore = "requires a PostgreSQL server with the migration extensions installed"]
async fn duplicate_projector_event_is_idempotent(pool: PgPool) {
    let event = event_envelope(1);
    let fragment = TestFragment {
        id: Uuid::now_v7(),
        value: 42,
    ...
    let make_envelope = || {
        ...
        ReadModelInvalidationEnvelope::try_new(
            &event,
            ProjectorName::new("test_projector"),
            [ReadModelDependency::Partition(partition)],
        )
        .expect("read-model invalidation should finalize")
    };
    ...
    let count = sqlx::query_scalar::<_, i64>("SELECT COUNT(*) FROM read_model_invalidation_outbox")
        .fetch_one(&pool)
        .await
