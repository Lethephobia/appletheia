...
impl PubsubCommandPublisher {
    ...
    fn build_message(command: &CommandEnvelope) -> Result<Message, PublisherError> {
        ...
        let ordering_key = command.ordering_key().to_string();
...
mod tests {
    use appletheia_application::{
        CausationId, CommandNameOwned, CommandOptions, CorrelationId, MessageId, SerializedCommand,
    };

    use super::*;

    #[test]
    fn publishes_envelope_json_with_native_attributes_and_ordering_key() {
        ...
        let message = PubsubCommandPublisher::build_message(&envelope).unwrap();
        let decoded: CommandEnvelope = serde_json::from_slice(&message.data).unwrap();
        assert_eq!(decoded, envelope);
        assert_eq!(message.attributes.get("command_name").unwrap(), "transfer");
        assert_eq!(
            message.attributes.get("message_id").unwrap(),
            &message_id.to_string()
        );
        assert!(!message.attributes.keys().any(|key| key.starts_with("ce-")));
        assert_eq!(message.ordering_key, envelope.correlation_id.to_string());
    }
}
