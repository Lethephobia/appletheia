...
use appletheia_application::event::{
    EventOutbox, EventOutboxDispatchError, EventOutboxLifecycle, EventOutboxWriter,
...
impl PgEventOutboxWriter {
    ...
    fn serialize_last_error(
        outbox: &EventOutbox,
    ) -> Result<Option<serde_json::Value>, EventOutboxWriterError> {
        match &outbox.last_error {
            Some(error) => {
                let json = serde_json::to_value(error as &EventOutboxDispatchError)
                    .map_err(|source| EventOutboxWriterError::Persistence(Box::new(source)))?;
                Ok(Some(json))
            }
            None => Ok(None),
        }
    ...
    async fn update_outbox_row(
        ...
        let transaction = uow.transaction_mut().map_err(|e| match e {
            UnitOfWorkError::NotInTransaction => EventOutboxWriterError::NotInTransaction,
            other => EventOutboxWriterError::Persistence(Box::new(other)),
        })?;
...

        {
            ...
            for outbox in dead_lettered_outboxes {
                ...
                let dead_lettered_at_value = match outbox.lifecycle {
                    EventOutboxLifecycle::DeadLettered { dead_lettered_at } => {
                        DateTime::<Utc>::from(dead_lettered_at)
                    }
                    EventOutboxLifecycle::Active => {
...
impl EventOutboxWriter for PgEventOutboxWriter {
    ...
    async fn write_outbox(
        ...
        for outbox in outboxes {
            if matches!(outbox.lifecycle, EventOutboxLifecycle::DeadLettered { .. }) {
