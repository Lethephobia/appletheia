...
use appletheia_application::command::{
    CommandOutbox, CommandOutboxDispatchError, CommandOutboxLifecycle, CommandOutboxWriter,
...
impl PgCommandOutboxWriter {
    ...
    ) -> Result<Option<serde_json::Value>, CommandOutboxWriterError> {
        match &outbox.last_error {
            Some(error) => {
                let json = serde_json::to_value(error as &CommandOutboxDispatchError)
                    .map_err(|source| CommandOutboxWriterError::Persistence(Box::new(source)))?;
                Ok(Some(json))
            }
            None => Ok(None),
        }
    ...
    async fn update_outbox_row(
        ...
        outbox: &CommandOutbox,
    ) -> Result<(), CommandOutboxWriterError> {
        ...
        let transaction = uow.transaction_mut().map_err(|e| match e {
            UnitOfWorkError::NotInTransaction => CommandOutboxWriterError::NotInTransaction,
            other => CommandOutboxWriterError::Persistence(Box::new(other)),
        })?;
...

        {
            ...
            for outbox in dead_lettered_outboxes {
                ...
                let dead_lettered_at_value = match outbox.lifecycle {
                    CommandOutboxLifecycle::DeadLettered { dead_lettered_at } => {
                        DateTime::<Utc>::from(dead_lettered_at)
                    }
                    CommandOutboxLifecycle::Active => Utc::now(),
                };
...
impl CommandOutboxWriter for PgCommandOutboxWriter {
    ...
    async fn write_outbox(
        ...
        for outbox in outboxes {
            if matches!(
                outbox.lifecycle,
                CommandOutboxLifecycle::DeadLettered { .. }
            ) {
