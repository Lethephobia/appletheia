...
/// Persists one read-model invalidation in its projection transaction.
...
impl ReadModelInvalidationOutboxEnqueuer for PgReadModelInvalidationOutboxEnqueuer {
    ...
    async fn enqueue_invalidation(
        ...
        let partitions =
            serde_json::to_value(&invalidation.invalidated_partitions).map_err(|source| {
        ...
        .await
        .map_err(|source| ReadModelInvalidationOutboxEnqueueError::Persistence(Box::new(source)))?;
