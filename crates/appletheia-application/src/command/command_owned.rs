...
use super::CommandOwnedError;

/// Represents an owned, serialized command with its execution options.
...
mod tests {
    ...
    fn try_from_command_serializes_command_name_and_payload() {
        let command_owned = CommandOwned::try_from_command(
            &TestCommand {
                value: "ok".to_owned(),
            },
    ...
    fn new_preserves_failure_reaction() {
        let mut failure_reaction = CommandFailureReaction::new();
        failure_reaction
            .push(
                &TestCommand {
