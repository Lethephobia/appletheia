...
use super::{
    Outbox, OutboxFetcher, OutboxFetcherAccess, OutboxPublishResult, OutboxPublisher,
    OutboxPublisherAccess, OutboxRelayConfigAccess, OutboxRelayError, OutboxRelayRunReport,
    OutboxState, OutboxWriter, OutboxWriterAccess,
};
use crate::unit_of_work::{UnitOfWork, UnitOfWorkError};

#[allow(async_fn_in_trait)]
pub trait OutboxRelay:
    OutboxRelayConfigAccess + OutboxPublisherAccess + OutboxFetcherAccess + OutboxWriterAccess
where
    ...
    <Self as OutboxWriterAccess>::Writer: OutboxWriter<Uow = Self::Uow>,
{
    ...
    async fn run_once(
        ...
        let operation_result: Result<Vec<Outbox>, OutboxRelayError> = async {
            ...
            for outbox in &mut outboxes {
                match &outbox.state {
                    ...
                    other => {
                        return Err(OutboxRelayError::NonPendingOutboxState(other.clone()));
                    }
                }
            }
        ...
        let mut outboxes: Vec<Outbox> = match operation_result {
            Ok(value) => {
                uow.commit().await?;
                value
            }
