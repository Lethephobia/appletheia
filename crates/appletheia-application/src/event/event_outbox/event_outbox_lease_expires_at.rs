...
use super::EventOutboxLeaseDuration;

#[derive(Copy, Clone, Debug, Eq, PartialEq, Hash, Ord, PartialOrd)]
pub struct EventOutboxLeaseExpiresAt(DateTime<Utc>);

impl EventOutboxLeaseExpiresAt {
    pub fn now() -> Self {
        Self(Utc::now())
    }
    ...
    pub fn from_now(expires_in: EventOutboxLeaseDuration) -> Self {
        Self(Utc::now() + expires_in.value())
    }
}

impl Default for EventOutboxLeaseExpiresAt {
    fn default() -> Self {
        Self::now()
    }
}

impl From<DateTime<Utc>> for EventOutboxLeaseExpiresAt {
    fn from(value: DateTime<Utc>) -> Self {
        Self(value)
    }
}

impl From<EventOutboxLeaseExpiresAt> for DateTime<Utc> {
    fn from(value: EventOutboxLeaseExpiresAt) -> Self {
        value.0
    }
}

impl Display for EventOutboxLeaseExpiresAt {
...
mod tests {
    ...
    fn default_uses_now() {
        let before = Utc::now();
        let lease_expires_at = EventOutboxLeaseExpiresAt::default();
        let after = Utc::now();
    ...
    fn value_returns_inner_datetime() {
        ...
        let lease_expires_at = EventOutboxLeaseExpiresAt::from(timestamp);
    ...
    fn conversions_round_trip() {
        ...
        let wrapped: EventOutboxLeaseExpiresAt = timestamp.into();
    ...
    fn from_now_adds_duration_to_now() {
        let backoff = EventOutboxLeaseDuration::from(Duration::seconds(30));
        let before = Utc::now();
        let lease_expires_at = EventOutboxLeaseExpiresAt::from_now(backoff);
        let after = Utc::now();
