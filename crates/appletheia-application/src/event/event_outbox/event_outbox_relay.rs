...
use super::{
    ...
    EventOutboxPublisher, EventOutboxPublisherAccess, EventOutboxRelayConfigAccess,
    EventOutboxRelayError, EventOutboxRelayRunReport, EventOutboxState, EventOutboxWriter,
    ...
    <Self as EventOutboxWriterAccess>::Writer: EventOutboxWriter<Uow = Self::Uow>,
{
    ...
    async fn run_forever(&self, uow: &mut Self::Uow) -> Result<(), EventOutboxRelayError> {
        ...
        while !self.is_stop_requested() {
            ...
            match run_report {
                EventOutboxRelayRunReport::Progress { .. } => {
                    poll_interval = polling_options.base;
                }
                EventOutboxRelayRunReport::Idle | EventOutboxRelayRunReport::Throttled => {
    ...
    async fn run_once(
        &self,
        uow: &mut Self::Uow,
    ) -> Result<EventOutboxRelayRunReport, EventOutboxRelayError> {
        ...
        if outboxes.is_empty() {
            return Ok(EventOutboxRelayRunReport::Idle);
        }
        ...
        Ok(EventOutboxRelayRunReport::Progress { proceeded })
    }
}
